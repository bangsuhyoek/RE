...
function SummaryCard({ subscriptions }) {
  ...
  return (
    <button type="button" onClick={() => setAnnual((value) => !value)} className="card-press w-full rounded-[24px] bg-[#18181B] p-5 text-left text-white" aria-label="월간 및 연간 지출 전환">
      ...
      <div className="mt-2 grid grid-cols-2 gap-4 border-t border-[#27272A] pt-3.5">
        <span>
...
function PromotionCarousel({ promotions, onOpen, onExplore }) {
  ...
  const typeLabels = ["01 더 저렴한 대체", "02 무료 · 이벤트", "03 연간 · 결합 할인"];
      ...
      <article className="overflow-hidden rounded-2xl border border-[#E4E4E7] bg-[#FAFAFA]">
        <div className="p-5">
          <div className="flex items-start justify-between gap-4">
            <span className="rounded-[4px] bg-black px-2 py-1 text-[11px] font-bold text-white">{typeLabels[index]}</span>
            <span className="grid h-9 w-9 place-items-center rounded-xl bg-[#18181B] text-[13px] font-bold text-white">{promo.monogram}</span>
          </div>
          <h3 className="mt-6 text-[19px] font-semibold tracking-[-0.02em]">{promo.title}</h3>
...
function EmptyState({ onAdd, onScan }) {
  return (
    <section className="flex min-h-[calc(100vh-9rem)] flex-col items-center justify-center px-5 text-center">
      ...
      <p className="mt-3 max-w-[280px] text-[14px] leading-6 text-[#71717A]">구독을 추가하면 다음 결제일과 절약할 수 있는 혜택을 한눈에 볼 수 있어요.</p>
...
export function HomeScreen({ subscriptions, promotions, profile, notificationDenied, onOpenSubscription, onShowAll, onOpenPromotion, onExplorePromotions, onAdd, onStartOnboarding }) {
  ...
  return (
    <main className="px-5 pb-28 pt-6">
      ...
      {notificationDenied && <button type="button" onClick={onStartOnboarding} className="mt-5 flex w-full items-center gap-3 rounded-xl border border-[#E4E4E7] bg-[#FAFAFA] px-4 py-3 text-left"><span className="grid h-8 w-8 place-items-center rounded-lg bg-white"><Sparkles size={16} /></span><span><strong className="block text-[13px]">결제 전 알림이 꺼져 있어요</strong><span className="mt-0.5 block text-[12px] text-[#71717A]">알림 권한을 켜고 D-3, D-1에 알려드릴게요.</span></span></button>}
      ...
      <section className="mt-8">
        <div className="mb-3 flex items-end justify-between">
          <div>
            ...
            <h2 className="text-[17px] font-bold tracking-[-0.01em]">결제 임박</h2>
          </div>
          {subscriptions.length > 3 && <button type="button" onClick={onShowAll} className="flex items-center gap-0.5 text-[12px] font-medium text-[#71717A] hover:text-black">전체보기 <ChevronRight size={15} /></button>}
        </div>
        <div className="space-y-3">
          {upcoming.map((subscription) => <SubscriptionCard key={subscription.subscriptionId} subscription={subscription} onOpen={() => onOpenSubscription(subscription.subscriptionId)} />)}
        </div>
      </section>
