    ...
    <main className="re-auth relative min-h-screen overflow-hidden px-6 pb-8 pt-8">
      ...
      <div className="relative z-10 flex min-h-[calc(100vh-4rem)] flex-col">
        <div className="flex items-center justify-between">
          <button type="button" onClick={onBack} className="re-icon-button" aria-label="이전 화면"><ArrowLeft size={20} /></button>
        ...
        <div className="mt-8 text-center">
          ...
          <p className="mt-3 text-[13px] leading-6 text-[#9099CA]">기존 로그인 기능은 그대로 두고<br />RE.의 새로운 화면으로 이어져요.</p>
        </div>

        <div className="mt-7 space-y-3">
          <Button className="w-full" onClick={() => onSocial("Apple", "")}>Apple로 계속하기 <ArrowRight size={17} /></Button>
          <Button variant="secondary" className="w-full" onClick={() => onSocial("Google", "")}><span className="grid h-5 w-5 place-items-center rounded-full border border-[#6371A9] text-[10px] font-bold">G</span> Google로 계속하기</Button>
        </div>
  ...
  return (
    <main className="re-auth relative min-h-screen overflow-hidden px-6 pb-10 pt-8">
      ...
      <div className="relative z-10">
        <div className="flex items-center justify-between"><button type="button" onClick={onBack} className="re-icon-button" aria-label="로그인으로 돌아가기"><ArrowLeft size={20} /></button><RELogo markClassName="h-[36px] w-auto" /><span className="w-10" /></div>
        <div className="mt-7 text-center"><p className="re-eyebrow">CREATE ACCOUNT</p><h1 className="re-serif-title mt-2 text-[27px] font-bold leading-[1.35] text-[#1B2A8C]">좋은 변화를<br />함께 시작해요.</h1><p className="mt-3 text-[13px] leading-6 text-[#9099CA]">기존 회원가입의 단계별 검증을 유지해요.</p></div>
