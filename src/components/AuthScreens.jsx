...
export function AuthRegister({ onBack, onComplete }) {
  ...
  const validation = useMemo(() => {
    const idFormat = idPattern.test(accountId);
    const passwordFormat = passwordPattern.test(password);
    const matching = Boolean(password) && password === passwordConfirm;
    const nicknameFormat = nicknamePattern.test(nickname);
    return {
      id: idFormat,
  ...
  const updateId = (value) => {
    setAccountId(value);
    if (idPattern.test(value)) setRevealedStep((current) => Math.max(current, 1));
  };

  const updatePassword = (value) => {
    setPassword(value);
    if (passwordPattern.test(value)) setRevealedStep((current) => Math.max(current, 2));
  };

  const updateConfirm = (value) => {
    setPasswordConfirm(value);
    if (value && passwordPattern.test(password) && value === password) {
  ...
  const updateNickname = (value) => {
    setNickname(value);
    if (nicknamePattern.test(value)) setRevealedStep((current) => Math.max(current, 4));
  };
  ...
  return (
    <main className="re-auth relative min-h-screen overflow-hidden px-6 pb-10 pt-8">
      ...
      <div className="relative z-10">
        <div className="flex items-center justify-between">
          <button type="button" onClick={onBack} className="re-icon-control" aria-label="로그인으로 돌아가기"><ArrowLeft size={20} /></button>
          <RELogo size="sm" />
          <span className="w-10" />
        </div>
        ...
        <div className="mt-7 text-center">
          ...
          <h1 className="re-serif-title mt-2 text-[27px] font-bold leading-[1.35] text-[#1B2A8C]">좋은 변화를<br />함께 시작해요.</h1>
          <p className="mt-3 text-[13px] leading-6 text-[#9099CA]">조건을 충족하면 다음 입력란이 자동으로 열려요.</p>
        </div>

        <form className="re-progressive-form mt-7 space-y-4" onSubmit={(event) => { event.preventDefault(); if (canSubmit) onComplete({ accountId, nickname }); }}>
          <label className="block">
            <span className="re-label">아이디</span>
            <span className="relative block">
              <UserRound className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-[#7E8AC0]" size={18} />
              <input className={`${fieldBase} pl-11`} autoComplete="username" placeholder="영문 소문자, 숫자, _- 5~20자" value={accountId} onChange={(event) => updateId(event.target.value)} />
            </span>
            <ValidationHint valid={validation.id} error={validation.idError} success={accountId && validation.id ? "사용 가능한 형식이에요. 실제 중복 확인은 계정 연동 시 처리해요." : ""} />
          </label>

          {revealedStep >= 1 && (
            <label className="re-progressive-field field-enter block" data-progressive-field="password">
              <span className="re-label">비밀번호</span>
              <span className="relative block">
                <LockKeyhole className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-[#7E8AC0]" size={18} />
                <input className={`${fieldBase} pl-11 pr-11`} type={showPassword ? "text" : "password"} autoComplete="new-password" placeholder="영문 대·소문자, 숫자, 특수문자" value={password} onChange={(event) => updatePassword(event.target.value)} />
                <button type="button" onClick={() => setShowPassword((value) => !value)} className="absolute right-3 top-1/2 -translate-y-1/2 rounded-lg p-2 text-[#7E8AC0]" aria-label={showPassword ? "비밀번호 숨기기" : "비밀번호 보기"}>{showPassword ? <EyeOff size={17} /> : <Eye size={17} />}</button>
              </span>
              <ValidationHint valid={validation.password} error={validation.passwordError} success={password && validation.password ? "사용 가능한 비밀번호예요." : ""} />
            </label>
          )}

          {revealedStep >= 2 && (
            <label className="re-progressive-field field-enter block" data-progressive-field="password-confirm">
              <span className="re-label">비밀번호 확인</span>
              <span className="relative block">
                <LockKeyhole className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-[#7E8AC0]" size={18} />
                <input className={`${fieldBase} pl-11 pr-11`} type={showConfirm ? "text" : "password"} autoComplete="new-password" placeholder="비밀번호를 한 번 더 입력해주세요" value={passwordConfirm} onChange={(event) => updateConfirm(event.target.value)} />
                <button type="button" onClick={() => setShowConfirm((value) => !value)} className="absolute right-3 top-1/2 -translate-y-1/2 rounded-lg p-2 text-[#7E8AC0]" aria-label={showConfirm ? "비밀번호 숨기기" : "비밀번호 보기"}>{showConfirm ? <EyeOff size={17} /> : <Eye size={17} />}</button>
              </span>
              <ValidationHint valid={validation.matching} error={validation.confirmError} success={passwordConfirm && validation.matching ? "동일한 비밀번호예요." : ""} />
            </label>
          )}

          {revealedStep >= 3 && (
            <label className="re-progressive-field field-enter block" data-progressive-field="nickname">
              <span className="re-label">닉네임</span>
              <input className={fieldBase} placeholder="한글 또는 영문 3~10자" value={nickname} onChange={(event) => updateNickname(event.target.value)} />
              <ValidationHint valid={validation.nickname} error={validation.nicknameError} success={nickname && validation.nickname ? "사용 가능한 닉네임이에요." : ""} />
            </label>
          )}

          {revealedStep >= 4 && (
            <Button type="submit" className="re-progressive-field field-enter mt-2 w-full" disabled={!canSubmit}>가입 완료</Button>
          )}
        </form>
      </div>
    </main>
