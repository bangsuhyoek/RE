...
public class PaymentCapturePlugin extends Plugin {
    ...
    public void simulatePayment(PluginCall call) {
        ...
        String body = call.getString("body",
                "정기 구독 결제가 완료되었습니다. 결제금액 19,000원 이용상품 유튜브 프리미엄");
        Boolean forceNew = call.getBoolean("forceNew", false);
        if (Boolean.TRUE.equals(forceNew)) {
            KnownSubscriptionSnapshotStore.syncEmptyForPoc(getContext());
        }

        PaymentParser.ParsedPayment parsed = PaymentParser.parse(pkg, title, body);
        JSObject result = new JSObject();
        if (parsed != null && parsed.isSubscription) {
            PaymentDetectionCoordinator.Result handled =
                    PaymentDetectionCoordinator.handle(getContext(), parsed);
            ...
            result.put("candidateId", handled.candidateId);
            result.put("eventType", handled.eventType);
            result.put("headsUpRequested", handled.headsUpRequested);
            result.put("overlayRequested", handled.overlayRequested);
        } else {
